...
namespace {
    ...
    public:
        explicit QByteArrayAppender(QByteArray &byteArray)
                : std::ostream(this),
                  std::streambuf(),
...
QByteArray
V2RayRustProfileGenerator::GenerateConfiguration(const ProfileContent &p, QMap<QString, QString>& tagProtocolMap)
{
...
void V2RayRustProfileGenerator::ProcessOutboundConfig(const OutboundObject &out)
{
    ...
    if(streamSettingsObject.network==u"ws"_qs){
        ...
        tbl.emplace<std::string>("uri",(u"ws://"_qs+out.outboundSettings.address+u":"_qs+QString::number(out.outboundSettings.port.from)+streamSettingsObject.wsSettings->path).toStdString());
