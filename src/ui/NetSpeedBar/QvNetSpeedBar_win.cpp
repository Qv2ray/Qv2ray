        ...
        namespace NetSpeedPlugin
        {
            namespace _win
            {
                ...
                void StartNamedPipeThread()
                {
                    ...
                    if (hThread == nullptr) {
                        LOG(MODULE_PLUGIN, "CreateThread failed, GLE=" + QString::number(GetLastError()))
                        return;
                ...
                DWORD WINAPI NamedPipeMasterThread(LPVOID lpvParam)
                {
                    ...
                    while (!isExiting) {
                        ...
                        if (hPipe == INVALID_HANDLE_VALUE) {
                            LOG(MODULE_PLUGIN, "CreateNamedPipe failed, GLE=" + QString::number(GetLastError()))
                    ...
                    while (!isExiting) {
                        ...
                        if (!fSuccess || cbBytesRead == 0) {
                            if (GetLastError() == ERROR_BROKEN_PIPE) {
                                LOG(MODULE_PLUGIN, "InstanceThread: client disconnected, GLE=" + QString::number(GetLastError()))
                            } else {
                                LOG(MODULE_PLUGIN, "InstanceThread ReadFile failed, GLE=" + QString::number(GetLastError()))
                            }
                        ...
                        if (!isExiting) {
                            ...
                            if (!fSuccess || cbReplyBytes != cbWritten) {
                                LOG(MODULE_PLUGIN, "InstanceThread WriteFile failed, GLE=" + QString::number(GetLastError()))
                                break;
                            }
                        }
                    }
