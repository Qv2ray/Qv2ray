...
namespace Qv2ray::components::latency::tcping
{
    ...
    TCPing::TCPing(std::shared_ptr<uvw::Loop> loop,LatencyTestRequest& req,LatencyTestHost* testHost)
    {
        ...
        for(int i=0;i<req.totalCount;++i)
        {
            ...
            tcpClient->once<uvw::ErrorEvent>([ptr=shared_from_this(),testHost,this,host=req.host,port=req.port,id=req.id](const uvw::ErrorEvent& e, uvw::TCPHandle&h) {
